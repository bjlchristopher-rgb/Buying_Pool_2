...
st.markdown("**CMHC Stress Test • Real Down Payments • Buyer Pool Comparison**")
...
def calc_stress_test_payment(price, contract_rate, first_time_buyer, new_construction):
    ...
    income_needed = monthly_payment * 12 / 0.39  # 39% GDS
...
with col3:
    ...
    first_time = st.checkbox("First-Time Buyer (30yr amort)", value=True)
    new_build = st.checkbox("New Construction (30yr amort)")
...
# ===========================================
# RESULTS + NEW BUYER DIFFERENCE METRIC
# ===========================================
...
with col_diff1:
    if abs(price1 - price2) > 50000:
        if buyer_difference > 0:
            st.metric("**🏆 Winner**", f"**Property 1**", f"+{buyer_difference:,.0f} buyers")
            st.success(f"**Property 1 wins with {buyer_difference:,.0f} more buyers**")
        elif buyer_difference < 0:
            st.metric("**🏆 Winner**", f"**Property 2**", f"+{abs(buyer_difference):,.0f} buyers")
            st.error(f"**Property 2 wins with {abs(buyer_difference):,.0f} more buyers**")
        else:
            st.warning("**Tie** - Equal buyer pools")
    else:
        st.info("**Same price** - Equal buyer pools")

with col_diff2:
    st.metric("**Buyer Difference**", f"{buyer_difference:,.0f}", 
              f"{(people1-people2)/max(people1,people2)*100:+.1f}%")
...
st.markdown("---")
with st.expander("📜 **CMHC Regulations Applied**"):
    st.markdown("""
